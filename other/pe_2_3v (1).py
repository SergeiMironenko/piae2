...
def model(x1,x2):
    return np.array([[1], [x1],[x2],[x1*x2], [x1*x1],[x2*x2], [x1**3], [x1**2 * x2], [x1 * x2**2], [x2**3]])
...
def fi(x, M):
    ...
    return -np.dot(f_value.T,  np.dot(M_inv, f_value))
...
def is_A_optimal(x, M):
    M_inv = np.linalg.inv(M)
    sigma = abs(fi(x, M)) * 0.01 # Должен быть множитель 0.01, но т.к. из-за такого условия рано
    ...
    if abs(-fi(x, M) + -np.trace(np.dot(M, M_inv))) <= sigma:
        return True
    return False
